...
def best_move(board, legal, color, turn):
    ...
    if best_score == (0, float('-inf')):
        best_score = random.randint(0,len(legal)-1)
        best_score = legal[best_score]
    return best_score
...
color = True
turn = 1
while not board.is_checkmate() or not chess.Board.is_insufficient_material():
    if color == True:
        print("Turn",turn)
        print("White's turn: ")
        turn = turn + 1
